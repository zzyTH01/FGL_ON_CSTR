...
Same method as plot_predictions.py (L=20, H=12), but at H=60 — a configuration
where the baseline MSE anomalously "collapses" to ~15 (predictions track targets
near-perfectly, corr≈0.94), which breaks FGL's premise (baseline ≤ teacher).
"""
...
b_corr = np.corrcoef(b_preds, b_targets)[0, 1]

print(f"\nResults:")
print(f"  Teacher  MSE (bin): {t_mse:.1f}")
print(f"  Baseline MSE (bin): {b_mse:.1f}   corr(pred,tgt)={b_corr:+.3f}")
...
fig.suptitle(f"CSTR H₂O Prediction — L={L}, H={H}, α={ALPHA}, T={TEMPERATURE} (seed={SEED})  "
             f"[baseline MSE collapse: b_mse={b_mse:.1f} < t_mse={t_mse:.1f}]",
             fontsize=13, fontweight='bold', y=1.01)
plt.tight_layout()

out_path = os.path.join(os.path.dirname(SCRIPT_DIR), "cstr_predictions_L20_H60.png")
...
out_path2 = os.path.join(os.path.dirname(SCRIPT_DIR), "cstr_scatter_L20_H60.png")
