...
2. **H2 支持(仅 τ=100)**:连续蒸馏地板 `≈ 0.76× baseline`(CV=0.21)——蒸馏是**固定比例降幅**,地板的决定因素不变。⚠️ 该比值**仅实测于 τ=100 单一数据集**,见下文 H2 适用范围。
...
跨 13 cells,c 均值 **0.76**(CV=0.21)。即连续蒸馏稳定地把地板降到 baseline 的 ~76%,比例在 (L,H) 上近似恒定。**(仅 τ=100 混沌延迟 CSTR,见下方适用范围。)**
...
> **适用范围(2026-08-06 修订):** 0.76 **仅实测于 τ=100 深挖网格**(13 cells×3 seeds);τ50/τ150 单锚点(L20H15)为 0.80/0.81,方向一致但非全网格;未在原始周期1 CSTR(可比单点 ≈0.98)或 MG(E 干涸无意义)上验证。"76 折券"暂为延迟混沌 CSTR 家族内规律。
