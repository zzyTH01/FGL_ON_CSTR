...
**关键洞察**：破坏周期性 ≠ 创造混沌。FGL 需要的不是"非周期性"，而是实际的反馈环复杂度差异。

---

## 9. Mackey-Glass τ 扫描——因果链的确立
...
**"混沌"不是 FGL 生效的必要条件——"反馈环 ≥ 2"才是。** 混沌只是创造了第二个反馈环的其中一种方式（且不是最有效的方式）。
...
**CSTR 不存在 τ 这个自由参数。** 这是它和 Mackey-Glass 之间最根本的结构性差异——不是某个参数没调好，而是 CSTR 的物理机理中根本没有可调节的反馈延迟。
...
所有四个配置在 τ≈13 附近均观测到强 FGL 正向（+36~49%），确认甜品区是 MG 方程自身的倍周期分岔结构的产物，与超参数 L、H 的选择无关。**"反馈环复杂度决定 FGL 效果"的理论通过了这次证伪检验。**
...
**"混沌 vs 周期"是表象。真正的自变量是系统中非线性反馈环的数量和耦合强度。** CSTR 失败不是因为它"太周期了"，而是因为 H₂/O₂ 燃烧机理只有一个非线性反馈环，且不存在可调参数来引入第二个——这是系统结构的固有限制，不是调参能突破的。

### 12.3 方法论意义

三个独立系统（CSTR 化学机理、Mackey-Glass DDE、Lorenz-63 ODE）收敛到同一个因果规律，构成了对 FGL 工作机制的完整理解。这个理解可以用来**预测**一个新数据集上 FGL 是否会有效——只需分析其非线性反馈环的数量——而不需要通过实验试错。

### 12.4 后续方向

1. 在更多经典系统上验证（Rössler、Chen 系统、Kuramoto-Sivashinsky），检验反馈环理论的普适性
