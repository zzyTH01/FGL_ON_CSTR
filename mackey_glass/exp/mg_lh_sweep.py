#!/usr/bin/env python
"""
MG τ=13 L×H sweep — mirror of CSTR and Lorenz sweeps.

Fixed τ=13 (period-doubling sweet spot). Grid: L∈{8,15,25,40,60}, H∈{5,15,30,45,60}.

Usage:
  cd /tmp && source .venv/bin/activate && python mackey_glass/exp/mg_lh_sweep.py
"""
...
TAU = 13
L_VALUES = [4, 7, 10, 13, 16]
H_VALUES = [4, 7, 10, 13, 16]
SEEDS = [0, 1, 2]
EPOCHS = 30
ALPHA, TEMPERATURE = 0.5, 4
NUM_BINS, PATIENCE, BATCH_SIZE = 50, 5, 128
N_POINTS = 10000
...
def run_fgl(L, H, data, seed):
    ...
    student=RNN(L,hs,os_,nl).to(device); opt_s=optim.Adam(student.parameters(),lr=lr); es_s=ES(PATIENCE)
    for _ in range(EPOCHS):
        student.train()
...
def main():
    ...
    csv_path=os.path.join(outdir,"mg_lh_sweep.csv")
    rows=[]; t0=time.time()
    ...
    grid_d=np.zeros((len(L_s),len(H_s))); grid_b=np.zeros_like(grid_d)
    ...
    for ax,grid,title,cmap in [
        (axes[0],grid_d,"FGL Δ%","RdYlGn"),
        (axes[1],grid_b,"Baseline MSE","YlOrRd"),
    ]:
    ...
    plt.suptitle(f"MG τ={TAU} L×H Sweep (period-doubling sweet spot)",fontweight="bold",y=1.01)
    plt.tight_layout()
    fig.savefig(os.path.join(outdir,"mg_lh_sweep.png"),dpi=150,bbox_inches="tight")
    plt.close(fig)
    ...
    print("\nTop 5:"); [print(f"  L={a['L']:2d} H={a['H']:2d}: Δ={a['delta']:+.1f}% Base={a['base']:.1f}") for a in all_aggs[:5]]
    print("Bottom 5:"); [print(f"  L={a['L']:2d} H={a['H']:2d}: Δ={a['delta']:+.1f}% Base={a['base']:.1f}") for a in all_aggs[-5:]]

    md_path=os.path.join(outdir,"mg_lh_sweep_report.md")
    with open(md_path,"w") as f:
        f.write(f"# MG τ={TAU} L×H Sweep\n\n")
        f.write(f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
        f.write(f"**Data:** MG τ={TAU} (period-doubling), {N_POINTS} pts\n")
        f.write(f"**L:** {L_VALUES}  |  **H:** {H_VALUES}\n")
        f.write(f"**Seeds:** {SEEDS}  |  **Epochs:** {EPOCHS}  |  **α:** {ALPHA}\n\n")
        f.write("## FGL Δ% Heatmap\n\n| L\\H | "+" | ".join(f"{h}" for h in H_s)+" |\n")
        f.write("|"+"---|"*(len(H_s)+1)+"\n")
        for L in L_s:
        ...
        f.write(f"\n## Data: `mg_lh_sweep.csv` ({len(rows)} rows)\n")
